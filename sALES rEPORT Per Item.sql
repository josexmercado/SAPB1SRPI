...
SELECT
	TRANSTYPE,
	TYPE as DocNumber,
	CASE WHEN TransType = 'Delivery' THEN CONCAT('IN ', BaseDocNum) 
	ELSE TYPE
	END AS DataGroup,
	...
	OCRCODE as Whse,
	CASE WHEN BaseType = -1 THEN CONCAT('IN ', BaseDocNum) 
	WHEN BaseType = 13 THEN CONCAT('IN ', BaseDocNum) 
	WHEN BaseType = 17 THEN CONCAT('SO ', BaseDocNum) 
	WHEN BaseType = 20 THEN 'GRPO'
	WHEN BaseType = 22 THEN CONCAT('PO ', BaseDocNum)
	END AS 'Base Document'

FROM
(	SELECT	
		1 as GBN,
		CASE WHEN T0.ISINS = 'Y' THEN CONCAT('IN ', T0.DOCNUM)
		...
		T1.StockSum - CASE WHEN T0.ISINS = 'Y' THEN	0
					  WHEN t0.U_BO_DRS = 'Y' THEN	0
					  ELSE	T1.STOCKVALUE END
		ELSE					
		T1.LineTotal - CASE WHEN T0.ISINS = 'Y' THEN	0
					   WHEN t0.U_BO_DRS = 'Y' THEN	0
					   ELSE	T1.STOCKVALUE END
		END AS GrossProfit,
		t1.OcrCode,
...
ORDER BY DataGroup, TaxDate,GBN ASC
